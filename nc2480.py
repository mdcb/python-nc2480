...
class hb_widget:
  def __init__(self, title, parent, lstart, cstart):
    pane = parent.subpanel(1, 3 + len(title) + 1, lstart, cstart)
    pane.win.clear()
    pane.win.addstr(0, 3, title, style.ERR)
    self.widget = text_widget(pane, 0, 1, 1)
    self.widget.win.addch(0, 0, curses.ACS_DIAMOND, style.ERR)
    self.pane = pane
    self.title = title
    self._beat = 0
    self._style = style.ERR
  ...
  def valid(self, state):
    ...
    self.pane.win.addstr(0, 3, self.title, self._style)
    self.pane.win.syncup()
    curses.panel.update_panels()
    curses.doupdate()
...
class pane:
  def __init__(self, pwin, lspan, cspan, lstart, cstart, title=None, boxed=True, **kwd):
    ...
    if title: self.win.addstr(0, 2, ' %s ' % title, title_attr)
  ...
  def settitle(self, title, style):
    if self.boxed:
      self.win.bkgdset(' ', style)
      self.win.box()
    self.win.addstr(0, 3, ' %s ' % title, style)
    #XXX#self.win.refresh()
    self.win.syncup() # XXX YEAH! XXX
    curses.panel.update_panels()
    curses.doupdate()
...
class top2480:
  ...
  def __init__(self, scrn, title=None, callback=None, poll10th=10, help_string=None, on_quit=None):
    ...
    try: curses.curs_set(0) # mac i-term
  ...
  def xtitle(self, title):
    print(']1;{}'.format(title)) # icon
    print(']2;{}'.format(title)) # window
    ...
    if k == -1:
      ...
      if k == curses.KEY_MOUSE:
        ...
        if not self.help_string:
          self.help_string = '? unrecognized options: ' + ', '.join([curses.keyname(k).decode() for k in self.key_callback_registry.keys()])
        #self.print_footer(curses.unctrl(k)+self.help_string)
        self.print_footer(curses.keyname(k).decode() + self.help_string)
  ...
  def trace_mouse_evt(self, win, evt, *args, **kwd):
    ...
    self.print_footer('l: %d c: %d' % (y, x), 1)
